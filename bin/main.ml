...
and mul_expr input = (chainl1 prim_expr (mulop <|> divop)) input
...
and stmt input =
  (if_stmt <|> while_stmt <|> assign_stmt) input
and if_stmt input =
...
and fn_stmt input =
  (let* name = token "fn" >> ident
   and* args = token "(" >> args
   and* body = token ")" >> token "{" >> stmts in
   token "}" >> return (FnDef(name, args, body))) input
